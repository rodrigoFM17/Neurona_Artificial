...
class GeneticAlgorithm:
    ...
    def get_i_generation_points(self, i):
        print(i)
        points = {
            "general": {
               "x": [],
            ...
            "best": {
                "x": self.best_subjects[i].get_x(),
                "y": self.best_subjects[i].get_y()
            },
            "worst": {
                "x": self.worst_subjects[i].get_x(),
                "y": self.worst_subjects[i].get_y()
            },
            "average_point":{
                "y": ( 
                    self.best_subjects[i].get_y() - self.worst_subjects[i].get_y()
                    ) / 2 + self.worst_subjects[i].get_y()
            }
        }

        for subject in self.generations[i]:
